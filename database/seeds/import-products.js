 ...
 * Importa bebidas, alimentos e condimentos para o banco de dados
 */
...
async function createCategories() {
  ...
  for (const category of categories) {
    ...
    if (error) {
      ...
      console.log(`✅ Categoria criada: ${category.name} (ID: ${data[0].id})`);
  ...
  for (const product of products) {
    try {
        ...
        .insert([
          {
            ...
            description: `${product.name} - Preço: R$ ${product.price.toFixed(2)}`,
            short_description: product.name,
            category_id: categoryId,
            price: product.price,
            cost_price: product.price * 0.6, // 60% do preço como custo
            stock_quantity: product.quantity,
            low_stock_threshold: 5,
            is_active: true,
            is_featured: product.price > 50 || product.quantity > 10, // Destaque para produtos caros ou com muito estoque
          },
        ])
        .select();
      ...
      if (error) {
        ...
        console.log(`✅ ${product.name} - R$ ${product.price.toFixed(2)} (Estoque: ${product.quantity})`);
        successCount++;
      }
    } catch (error) {
